...
def df_add_phases(
    ...
    df['phase_target_trial_num'] = df.groupby(['ppid_full', 'phase', 'target_x_label'], observed=True).cumcount() + 1
    
    df['phase_trial_num'] = df.groupby(['ppid_full', 'phase'],  observed=True).cumcount() + 1
